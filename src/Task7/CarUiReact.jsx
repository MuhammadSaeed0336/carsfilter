...
const CarUiReact = (props) => {
    let {filterCar, comCar, cars, filterModel, modelCar, filterColor, carCol} = props;
  return (
    <>
    <div className="row">
          ...
          <div className="menu-tab d-grid gap-2">   
            {
              comCar.map((curElem, id)=>{
                return <button className='btn btn-dark' key={id} onClick={()=>filterCar(curElem)}>{curElem}</button>
              })
            }
          </div>
      </div>
          ...
          <div className="menu-tab d-flex flex-wrap gap-2">
            {
              modelCar.map((curElem, id)=>{
                return <button className='btn btn-dark' key={id} onClick={()=>filterModel(curElem)}>{curElem}</button>
              })
            }
          </div>
      </div>
          ...
          <div className="menu-tab  d-flex flex-wrap gap-2 ">
            {
              carCol.map((curElem, id)=>{
                return <button className='btn btn-warning'key={id} onClick={()=>filterColor(curElem)}>{curElem}</button>
              })
            }
          </div>
      </div>
